...
export default function RepoPage({ params }: { params: any }) {
    ...
    const getRepository = async () => {
        ...
        try {
            ...
            setIsOwner(repositoryRes.data.ownerAddress.toLowerCase() === account.address.toLowerCase());
        } catch (error) {
            console.error(error);
        } finally {
            setLoading(false);
    ...
    return <div className="h-screen flex flex-col py-36 px-8">
        <div className="flex flex-col md:flex-row space-y-4 space-x-0 md:items-center md:space-x-4 md:space-y-0">
            ...
            <button
                type="button"
                onClick={() => navigator.clipboard.writeText(`https://gitgate.github.com/repositories/${repository._id}/invite`)}
            ...
            <div className="flex flex-col">
                ...
                <div className="bg-gray-200 h-2 w-64 rounded-lg">
                    <div className="bg-green-600 h-2 rounded-lg" style={{width: "97%"}}></div>
                </div>
            </div>
        </div>
        <hr className="my-4"/>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="flex flex-col">
                <h3 className="text-xl font-semibold">Tokens</h3>
                    {
                        repository.requirements.length === 0  && <p className="mt-2 text-gray-300">No token requirement!</p>
                    }
                <div className="grid grid-cols-2 md:grid-cols-3 mt-2 gap-4">
                    { repository.requirements.map((address: string) => {
                        return (
                            <span onClick={() => push(`/profile/${address}`)} className="inline-flex transition-transform hover:scale-105 text-center w-auto cursor-pointer items-center justify-center rounded-md bg-purple-100 px-2.5 py-0.5 text-sm font-medium text-purple-800">
            ...
            <div className="flex flex-col">
                ...
                {
                    repository.blacklistedAddresses.filter((address: string) => address !== '').length === 0 && <p className="mt-2 text-gray-300">No blacklisted address!</p>
                }
                <div className="grid grid-cols-1 mt-2 gap-4">
                    { repository.blacklistedAddresses.filter((address: string) => address !== '').map((address: string) => {
                        return (
                            <span onClick={() => push(`/profile/${address}`)} className="inline-flex transition-transform hover:scale-105 text-center w-auto cursor-pointer items-center rounded-md bg-gray-100 px-2.5 py-0.5 text-sm font-medium text-gray-800">
                                { shortenHex(address) }
                            </span>
                        )
                    })}
